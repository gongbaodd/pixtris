...
type ShapeRotation = Position[];
type ShapeType = 'i' | 'j' | 'l' | 'o' | 's' | 't' | 'z';
type ShapeColor = 'blue' | 'green' | 'purple' | 'yellow' | 'cyan' | 'orange' | 'red';

// each shape type has 4 rotations, each 4 block position [row,col] on 4x4 shape grid
export const SHAPES: Record<ShapeType, ShapeRotation[]> = {
    i: [[[0,1],[1,1],[2,1],[3,1]],
        [[1,0],[1,1],[1,2],[1,3]],
        [[0,2],[1,2],[2,2],[3,2]],
        [[2,0],[2,1],[2,2],[2,3]]],
...
export const SHAPE_COLORS: Record<ShapeType, ShapeColor> = {
    j: 'blue', 
    s: 'green', 
    t: 'purple', 
    o: 'yellow', 
...
export default class Tetromino {
     ...
     */
    absolutePos(shiftRow: number = 0, shiftCol: number = 0, rotate: boolean = false): Position[] {
